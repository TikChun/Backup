...
int main(){
    ...
    while(true){
        ...
        vpDisplay::display(I);
        if(!init_done){
            vpDisplay::displayText(I,vpImagePoint(10,10), "Click in the blob to initializa the tracker", vpColor::red);
...
//g++ -g `/home/tikchuntong/visp-ws/visp-build/bin/visp-config --cflags` -o realsense_blob_tracking realsense_blob_tracking.cpp `/home/tikchuntong/visp-ws/visp-build/bin/visp-config --libs` $(pkg-config --cflags --libs opencv4) -lrealsense2