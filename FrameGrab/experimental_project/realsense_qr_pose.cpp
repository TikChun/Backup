...
// #include "/home/tikchuntong/visp-ws/visp/tutorial/computer-vision/pose_helper.h"
...
void computePose(std::vector<vpPoint> &point, const std::vector<vpImagePoint> &ip, const vpCameraParameters &cam, bool init, vpHomogeneousMatrix &cMo){
    ...
    if(init) pose.computePose(vpPose::DEMENTHON_LAGRANGE_VIRTUAL_VS, cMo);
...
int main()
{
    ...
    while (true)
    {
        ...
        // std::cout << " width : "<< color_frame.get_width() << std::endl;
        // std::cout << " height : "<< color_frame.get_height() << std::endl;
        ...
        if (init_uncomplete)
        {
            ...
            vpDisplay::display(I);
            vpDisplay::flush(I);
            cam = vpCameraParameters(1280,720,I.getWidth() / 2, I.getHeight() / 2);
...
// g++ -g `/home/tikchuntong/visp-ws/visp-build/bin/visp-config --cflags` -o realsense_qr_pose realsense_qr_pose.cpp `/home/tikchuntong/visp-ws/visp-build/bin/visp-config --libs` $(pkg-config --cflags --libs opencv4) -lrealsense2